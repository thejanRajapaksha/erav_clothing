<?php
class Banktypeinfo extends CI_Model{
    public function Banktypeinsertupdate(){
        ...
        if($recordOption==1){
            ...
            $this->db->where('idtbl_bank', $recordID);
    ...
    public function Banktypeedit(){
        ...
        $obj=new stdClass();
        $obj->id=$respond->row(0)->idtbl_bank;
        $obj->bname=$respond->row(0)->bname;
        echo json_encode($obj);
    }

    public function GetBanktypeList(){
        $this->db->select('idtbl_bank, bname');
        $this->db->from('tbl_bank');
        $this->db->where('status', 1);
